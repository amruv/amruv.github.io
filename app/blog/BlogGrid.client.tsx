      ...
      <div className="relative z-30 overflow-y-auto pb-16">
        ...
        <CardContent className={compact ? 'pb-4 px-5' : 'pb-4'}>
          ...
          <div className="flex flex-wrap gap-1 mb-4">
            {(compact ? [post.partTag, ...(post.tags || []).slice(0, 2)].filter(Boolean) : post.tags)?.map((tag, tagIndex) => (
  ...
  return (
    ...
    >
      <div className="flex gap-4 items-stretch relative w-full">
        {/* Collapse/Expand button to the left of the series, independent of the parent card */}
        <motion.button
          type="button"
          whileTap={{ scale: 0.92 }}
          onClick={() => setIsExpanded((current) => !current)}
          className="flex items-center justify-center w-8 h-8 rounded-full border border-accent/40 bg-card hover:bg-accent/10 text-accent transition-all duration-300 shadow-sm shrink-0 self-center focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-background"
        ...
        {/* Parent card wrapper */}
        <motion.div
          className="relative z-10 w-full md:w-[280px] lg:w-[320px] flex-none"
        >
          <BlogPostCard
            post={parent}
            footerAction={
